...
func main() {
   fmt.Println("Table of Prime Factors of 2 to 100")
   fmt.Println("Where ^ and * are the exponentiation and multiplication operators respectively")
   fmt.Println("")
   var i uint64
   var row string
   for i = 2; i <= 100; i++ {
      row = strconv.FormatUint(i, 10) + " = "
      pf, _ := PrimeFactors(i)

      for prime, exp := range pf {
         row += strconv.FormatUint(prime, 10)
         if exp > 1 {
            row += "^" + strconv.FormatUint(exp, 10)
         }
         row += "*"
      }
